...
exports.PARAMETRO_SERVICIO_PROVIDER = exports.ParametroService = void 0;
...
let ParametroService = exports.ParametroService = class ParametroService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.ParametroFiltro();
                filtro.tipo = objetoDto.tipo;
                filtro.valor = objetoDto.valor;
                filtro.texto = objetoDto.texto;
                const parametroBD = await this.repositorioFactory.parametroRepositorio.obtenerObjetoPor(filtro);
                if (parametroBD) {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.parametroRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.ParametroFiltroDto, filtros_1.ParametroFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Parametro, transferencia_2.ParametroDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.parametroRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Parametro, transferencia_2.ParametroDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ParametroCreacionDto, entidades_1.Parametro);
            await this.repositorioFactory.parametroRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Parametro, transferencia_2.ParametroDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ParametroModificacionDto, entidades_1.Parametro);
            await this.repositorioFactory.parametroRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha modificado con éxito.', this.mapper.map(objeto, entidades_1.Parametro, transferencia_2.ParametroDto));
        }
        catch (error) {
            await this.repositorioFactory.revertir(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Excepcion, error.toString(), null);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.parametroRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
...
exports.ParametroService = ParametroService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(persistencia_1.REPOSITORIO_FACTORY)),
    __param(1, (0, nestjs_1.InjectMapper)()),
    __metadata("design:paramtypes", [Object, Object])
], ParametroService);
exports.PARAMETRO_SERVICIO_PROVIDER = {
    provide: servicios_1.PARAMETRO_SERVICIO,
