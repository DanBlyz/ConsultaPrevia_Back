...
var ProvidenciaRepository_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.PROVIDENCIA_REPOSITORIO_PROVIDER = exports.ProvidenciaRepository = void 0;
...
let ProvidenciaRepository = ProvidenciaRepository_1 = class ProvidenciaRepository {
    constructor(conexion, mapper) {
        ...
        this.mapper = mapper;
        this.repositorio = this.conexion.getRepository(orm_1.ProvidenciaEntity);
    }
    ...
    async obtenerPorId(id) {
        ...
        return respuesta
            ? this.mapper.map(respuesta, orm_1.ProvidenciaEntity, entidades_1.Providencia)
            : null;
    }
    ...
    async obtenerPor(filtro, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        ...
        return new modelos_1.ListaPaginada(this.mapper.mapArray(respuesta[0], orm_1.ProvidenciaEntity, entidades_1.Providencia), respuesta[1]);
    }
    async guardar(objeto, transaccion) {
        ...
        return await transaccion.manager
            ...
            .catch((error) => {
            __1.RepositorioFactory.registrarError(ProvidenciaRepository_1.name, error);
            return 0;
        });
    }
    async modificar(id, objeto, transaccion) {
        const objetoEntity = await transaccion.manager.preload(orm_1.ProvidenciaEntity, Object.assign({ id }, objeto));
        return await transaccion.manager
            .update(orm_1.ProvidenciaEntity, { id }, objetoEntity)
            .then(() => true)
            .catch((error) => {
            __1.RepositorioFactory.registrarError(ProvidenciaRepository_1.name, error);
            return false;
    ...
    async eliminar(id, transaccion, borradoFisico = false) {
        if (borradoFisico) {
            return await transaccion.manager
                .delete(orm_1.ProvidenciaEntity, { id })
                .then(() => true)
                .catch((error) => {
                __1.RepositorioFactory.registrarError(ProvidenciaRepository_1.name, error);
                return false;
            ...
            const objetoEntity = await transaccion.manager.preload(orm_1.ProvidenciaEntity, {
                id,
            });
...
//# sourceMappingURL=providencia.repository.js.map