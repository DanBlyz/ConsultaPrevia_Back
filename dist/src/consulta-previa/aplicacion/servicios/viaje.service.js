...
exports.VIAJE_SERVICIO_PROVIDER = exports.ViajeService = void 0;
...
let ViajeService = class ViajeService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.ViajeFiltro();
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.viajeRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.ViajeFiltroDto, filtros_1.ViajeFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Viaje, transferencia_2.ViajeDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.viajeRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Viaje, transferencia_2.ViajeDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ViajeCreacionDto, entidades_1.Viaje);
            await this.repositorioFactory.viajeRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Viaje, transferencia_2.ViajeDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ViajeModificacionDto, entidades_1.Viaje);
            await this.repositorioFactory.viajeRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha modificado con éxito.', this.mapper.map(objeto, entidades_1.Viaje, transferencia_2.ViajeDto));
        }
        catch (error) {
            await this.repositorioFactory.revertir(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Excepcion, error.toString(), null);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.viajeRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
