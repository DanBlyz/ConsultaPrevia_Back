...
exports.CLASIFICACION_SERVICIO_PROVIDER = exports.ClasificacionService = void 0;
...
let ClasificacionService = class ClasificacionService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.ClasificacionFiltro();
                filtro.nombre = objetoDto.nombre;
                filtro.descripcion = objetoDto.descripcion;
                const clasificacionBD = await this.repositorioFactory.clasificacionRepositorio.obtenerObjetoPor(filtro);
                if (clasificacionBD) {
                    errores.push('El número de clasificacion ya existe.');
                    return false;
                }
            }
            case 'modificar': {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.clasificacionRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.ClasificacionFiltroDto, filtros_1.ClasificacionFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Clasificacion, transferencia_2.ClasificacionDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.clasificacionRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Clasificacion, transferencia_2.ClasificacionDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ClasificacionCreacionDto, entidades_1.Clasificacion);
            await this.repositorioFactory.clasificacionRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Clasificacion, transferencia_2.ClasificacionDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ClasificacionModificacionDto, entidades_1.Clasificacion);
            await this.repositorioFactory.clasificacionRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha modificado con éxito.', this.mapper.map(objeto, entidades_1.Clasificacion, transferencia_2.ClasificacionDto));
        }
        catch (error) {
            await this.repositorioFactory.revertir(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Excepcion, error.toString(), null);
...
ClasificacionService = __decorate([
...
exports.CLASIFICACION_SERVICIO_PROVIDER = {
    provide: servicios_1.CLASIFICACION_SERVICIO,
    useClass: ClasificacionService,
};
