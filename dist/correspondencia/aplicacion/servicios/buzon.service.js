...
exports.BUZON_SERVICIO_PROVIDER = exports.BuzonService = void 0;
...
let BuzonService = class BuzonService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.BuzonFiltro();
                filtro.uniOrganizacional = objetoDto.uniOrganizacional;
                filtro.puesto = objetoDto.puesto;
                filtro.estado = objetoDto.estado;
                const buzonBD = await this.repositorioFactory.buzonRepositorio.obtenerObjetoPor(filtro);
                if (buzonBD) {
                    errores.push('El número de buzon ya existe.');
                    return false;
                }
            }
            case 'modificar': {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.buzonRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.BuzonFiltroDto, filtros_1.BuzonFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Buzon, transferencia_2.BuzonDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.buzonRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Buzon, transferencia_2.BuzonDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.BuzonCreacionDto, entidades_1.Buzon);
            await this.repositorioFactory.buzonRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Buzon, transferencia_2.BuzonDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.BuzonModificacionDto, entidades_1.Buzon);
            await this.repositorioFactory.buzonRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha modificado con éxito.', this.mapper.map(objeto, entidades_1.Buzon, transferencia_2.BuzonDto));
        }
        catch (error) {
            await this.repositorioFactory.revertir(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Excepcion, error.toString(), null);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.buzonRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
...
exports.BuzonService = BuzonService;
exports.BUZON_SERVICIO_PROVIDER = {
    provide: servicios_1.BUZON_SERVICIO,
    useClass: BuzonService,
};
//# sourceMappingURL=buzon.service.js.map