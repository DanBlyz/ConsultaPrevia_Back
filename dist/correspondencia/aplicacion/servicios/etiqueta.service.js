...
exports.ETIQUETA_SERVICIO_PROVIDER = exports.EtiquetaService = void 0;
...
let EtiquetaService = class EtiquetaService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.EtiquetaFiltro();
                filtro.nombre = objetoDto.nombre;
                filtro.descripcion = objetoDto.descripcion;
                filtro.usuario = objetoDto.usuario;
                filtro.esPublica = objetoDto.esPublica;
                const etiquetaBD = await this.repositorioFactory.etiquetaRepositorio.obtenerObjetoPor(filtro);
                if (etiquetaBD) {
                    errores.push('El número de etiqueta ya existe.');
                    return false;
                }
            }
            case 'modificar': {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.etiquetaRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.EtiquetaFiltroDto, filtros_1.EtiquetaFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Etiqueta, transferencia_2.EtiquetaDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.etiquetaRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Etiqueta, transferencia_2.EtiquetaDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.EtiquetaCreacionDto, entidades_1.Etiqueta);
            await this.repositorioFactory.etiquetaRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Etiqueta, transferencia_2.EtiquetaDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.EtiquetaModificacionDto, entidades_1.Etiqueta);
            await this.repositorioFactory.etiquetaRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha modificado con éxito.', this.mapper.map(objeto, entidades_1.Etiqueta, transferencia_2.EtiquetaDto));
        }
        catch (error) {
            await this.repositorioFactory.revertir(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Excepcion, error.toString(), null);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.etiquetaRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
...
exports.ETIQUETA_SERVICIO_PROVIDER = {
    provide: servicios_1.ETIQUETA_SERVICIO,
    useClass: EtiquetaService,
};
