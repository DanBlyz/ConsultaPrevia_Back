...
exports.PRUEBA_SERVICIO_PROVIDER = exports.PruebaService = void 0;
...
let PruebaService = class PruebaService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.PruebaFiltro();
                filtro.id = objetoDto.id;
                filtro.nombre = objetoDto.nombre;
                filtro.apellido = objetoDto.apellido;
                const pruebaBD = await this.repositorioFactory.pruebaRepositorio.obtenerObjetoPor(filtro);
                if (pruebaBD) {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.pruebaRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.PruebaFiltroDto, filtros_1.PruebaFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Prueba, transferencia_2.PruebaDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.pruebaRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Prueba, transferencia_2.PruebaDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.PruebaCreacionDto, entidades_1.Prueba);
            await this.repositorioFactory.pruebaRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Prueba, transferencia_2.PruebaDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.PruebaModificacionDto, entidades_1.Prueba);
            await this.repositorioFactory.pruebaRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha modificado con éxito.', this.mapper.map(objeto, entidades_1.Prueba, transferencia_2.PruebaDto));
        }
        catch (error) {
            await this.repositorioFactory.revertir(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Excepcion, error.toString(), null);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.pruebaRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
...
exports.PRUEBA_SERVICIO_PROVIDER = {
    provide: servicios_1.PRUEBA_SERVICIO,
