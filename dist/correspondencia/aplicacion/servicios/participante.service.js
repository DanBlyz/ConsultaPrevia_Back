...
exports.PARTICIPANTE_SERVICIO_PROVIDER = exports.ParticipanteService = void 0;
...
let ParticipanteService = class ParticipanteService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.ParticipanteFiltro();
                ...
                const participanteBD = await this.repositorioFactory.participanteRepositorio.obtenerObjetoPor(filtro);
                if (participanteBD) {
                    errores.push('El número de participante ya existe.');
                    return false;
                }
            }
            case 'modificar': {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.participanteRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.ParticipanteFiltroDto, filtros_1.ParticipanteFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Participante, transferencia_2.ParticipanteDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.participanteRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Participante, transferencia_2.ParticipanteDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ParticipanteCreacionDto, entidades_1.Participante);
            await this.repositorioFactory.participanteRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Participante, transferencia_2.ParticipanteDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ParticipanteModificacionDto, entidades_1.Participante);
            await this.repositorioFactory.participanteRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha modificado con éxito.', this.mapper.map(objeto, entidades_1.Participante, transferencia_2.ParticipanteDto));
        }
        catch (error) {
            await this.repositorioFactory.revertir(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Excepcion, error.toString(), null);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.participanteRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
...
exports.PARTICIPANTE_SERVICIO_PROVIDER = {
    provide: servicios_1.PARTICIPANTE_SERVICIO,
    useClass: ParticipanteService,
};
