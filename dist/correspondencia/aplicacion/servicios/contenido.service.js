...
exports.CONTENIDO_SERVICIO_PROVIDER = exports.ContenidoService = void 0;
...
let ContenidoService = class ContenidoService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.ContenidoFiltro();
                filtro.version = objetoDto.version;
                const contenidoBD = await this.repositorioFactory.contenidoRepositorio.obtenerObjetoPor(filtro);
                if (contenidoBD) {
                    errores.push('El número de contenido ya existe.');
                    return false;
                }
            }
            case 'modificar': {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.contenidoRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.ContenidoFiltroDto, filtros_1.ContenidoFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Contenido, transferencia_2.ContenidoDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.contenidoRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Contenido, transferencia_2.ContenidoDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ContenidoCreacionDto, entidades_1.Contenido);
            await this.repositorioFactory.contenidoRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Contenido, transferencia_2.ContenidoDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.ContenidoModificacionDto, entidades_1.Contenido);
            await this.repositorioFactory.contenidoRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.contenidoRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
...
exports.CONTENIDO_SERVICIO_PROVIDER = {
    provide: servicios_1.CONTENIDO_SERVICIO,
    useClass: ContenidoService,
};
