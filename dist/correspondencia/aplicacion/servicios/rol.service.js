...
let RolService = class RolService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.RolFiltro();
                filtro.codigo = objetoDto.codigo;
                filtro.nombre = objetoDto.nombre;
                const rolBD = await this.repositorioFactory.rolRepositorio.obtenerObjetoPor(filtro);
                if (rolBD) {
                    errores.push('El número de rol ya existe.');
                    return false;
                }
            }
            case 'modificar': {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.rolRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.RolFiltroDto, filtros_1.RolFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Rol, transferencia_2.RolDto), respuesta.totalRegistros);
    }
    ...
    async guardar(objetoDto) {
        ...
        try {
            ...
            await this.repositorioFactory.rolRepositorio.guardar(objeto, transaccion);
    ...
    async modificar(id, objetoDto) {
        ...
        try {
            ...
            await this.repositorioFactory.rolRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.rolRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
