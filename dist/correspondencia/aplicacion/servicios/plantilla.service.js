...
exports.PLANTILLA_SERVICIO_PROVIDER = exports.PlantillaService = void 0;
...
let PlantillaService = class PlantillaService {
    ...
    async validar(operacion, objetoDto, errores = []) {
        switch (operacion) {
            case 'guardar': {
                const filtro = new filtros_1.PlantillaFiltro();
                filtro.version = objetoDto.version;
                const plantillaBD = await this.repositorioFactory.plantillaRepositorio.obtenerObjetoPor(filtro);
                if (plantillaBD) {
    ...
    async buscar(filtroDto, pagina, cantidad, ordenarPor = 'id', orden = 'DESC') {
        const respuesta = await this.repositorioFactory.plantillaRepositorio.obtenerPor(this.mapper.map(filtroDto, filtros_2.PlantillaFiltroDto, filtros_1.PlantillaFiltro), pagina, cantidad, ordenarPor, orden);
        return new transferencia_1.ListaPaginadaDto(this.mapper.mapArray(respuesta.lista, entidades_1.Plantilla, transferencia_2.PlantillaDto), respuesta.totalRegistros);
    }
    async obtenerPorId(id) {
        const objeto = await this.repositorioFactory.plantillaRepositorio.obtenerPorId(id);
        if (objeto === undefined)
            return null;
        return this.mapper.map(objeto, entidades_1.Plantilla, transferencia_2.PlantillaDto);
    }
    async guardar(objetoDto) {
        ...
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.PlantillaCreacionDto, entidades_1.Plantilla);
            await this.repositorioFactory.plantillaRepositorio.guardar(objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha guardado con éxito.', this.mapper.map(objeto, entidades_1.Plantilla, transferencia_2.PlantillaDto));
        }
        catch (error) {
    ...
    async modificar(id, objetoDto) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            const objeto = this.mapper.map(objetoDto, transferencia_2.PlantillaModificacionDto, entidades_1.Plantilla);
            await this.repositorioFactory.plantillaRepositorio.modificar(id, objeto, transaccion);
            await this.repositorioFactory.confirmar(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Exito, 'El registro se ha modificado con éxito.', this.mapper.map(objeto, entidades_1.Plantilla, transferencia_2.PlantillaDto));
        }
        catch (error) {
            await this.repositorioFactory.revertir(transaccion);
            return new transferencia_1.RespuestaObjetoDto(transferencia_1.TipoRespuesta.Excepcion, error.toString(), null);
    ...
    async eliminar(id) {
        ...
        const transaccion = await this.repositorioFactory.iniciarTransaccion();
        try {
            await this.repositorioFactory.plantillaRepositorio.eliminar(id, transaccion, false);
            await this.repositorioFactory.confirmar(transaccion);
...
exports.PLANTILLA_SERVICIO_PROVIDER = {
    provide: servicios_1.PLANTILLA_SERVICIO,
    useClass: PlantillaService,
};
